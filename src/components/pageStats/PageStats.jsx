...
function PageStats() {
    ...
    useEffect(() => {
        console.log(stats)
        if (stats === 10) {
            aggregateInCollection('posts', [{
                $lookup: {
                    from: 'user_worker', localField: 'user_id', foreignField: '_id', as: 'worker_data',
                },
            }, {
                $lookup: {
                    from: 'user_enterprise', localField: 'user_id', foreignField: '_id', as: 'enterprise_data',
                },
            }, {
                $group: {
                    _id: '$user_id', posts_count: { $sum: 1 }, worker_data: { $first: '$worker_data' }, enterprise_data: { $first: '$enterprise_data' },
                },
            }, { $sort: { posts_count: -1 } }, {
                $project: {
                    user_id: '$_id', posts_count: 1, user_name: { $cond: { if: { $gt: [{ $size: '$worker_data' }, 0] }, then: '$worker_data.name', else: '$enterprise_data.name' } }, _id: 0,
                },
            }, { $limit: 10 }]).then((res) => {
            ...
            aggregateInCollection('user_worker', [{ $project: { name: 1, last_name: { $arrayElemAt: [{ $split: ['$name', ' '] }, 1] }, first_name_length: { $strLenCP: { $arrayElemAt: [{ $split: ['$name', ' '] }, 0] } } } }, { $group: { _id: '$last_name', count: { $sum: 1 }, avg_name_length: { $avg: '$first_name_length' } } }, { $sort: { count: -1 } }, { $limit: 10 }, {
                $project: {
                    lastname: '$_id', count: 1, avg_name_length: 1, _id: 0,
                },
            }]).then((res) => {
                setResult(res.documents)
